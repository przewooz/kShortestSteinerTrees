...
// is high enough
void Record::add(float fitness, float unitation, int evals, time_t time, const vector<bool>& solution) {
  if (verbosity > 1) {
    std::cout << fitness << "\t" << unitation << "\t" << evals << "\t" << time << std::endl;
  }
  levels.push_back(tuple<float, float, int, time_t, vector<bool>>(fitness, unitation, evals, time, solution));
}

// Returns information about the best fitness reached
const tuple<float, float, int, time_t, vector<bool>>& Record::best() const {
...
array<int, 3> Record::summarize(const vector<Record>& records, Configuration& config) {
  ...
  for (const auto& record : records) {
    ...
    if (std::get<0>(best) >= good_enough) {
      evals.push_back(std::get<1>(best));
    } else {
      result[FAILURES]++;
    }
  }
