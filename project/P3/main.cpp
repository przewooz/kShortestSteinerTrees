...
int main(int argc, char * argv[]) {
  ...
  config.set("problem", string("Injected"));
  ...
  if (config.get<string>("experiment") == "bisection") {
    ...
    for (const auto& record : records) {
      if (config.get<int>("verbosity") > 1) {
        cout << std::get<0>(record.best()) << " " << std::get<1>(record.best()) << " " << std::get<2>(record.best()) << " " << std::get<3>(record.best()) << endl;
      }
      out << std::get<0>(record.best()) << " " << std::get<1>(record.best()) << " " << std::get<2>(record.best()) << " " << std::get<3>(record.best()) << endl;
    }
    out.close();
  } else { // single_run
    Record record = single_run(rand, config, problem, optimizer_method, 0, nullptr);
    ofstream out(dat_file.c_str());
    for (const auto& line : record.progression()) {
      out << std::get<0>(line) << " " << std::get<1>(line) << " " << std::get<2>(line) << " " << std::get<3>(line) << endl;
    }
    out.close();
  }
  return 0;
